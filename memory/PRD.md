...
- ✅ **Logo + favicon Voxyra (07/07/2026)**: recebido asset `login_branding_alpha.png` (720×720). Salvo em `/app/frontend/public/logo.png` (original), gerado `favicon-32.png` (32×32), `apple-touch-icon.png` (180×180) e `favicon.ico` multi-size via Pillow. `index.html` atualizado com `link rel=icon/apple-touch-icon` apontando para os arquivos gerados + `theme-color=#1D6BFF`. Substituído em 4 pontos da UI: (1) `ClientLogin.jsx` — `BrandMark` agora usa `/logo.png` diretamente (removido fallback do G-colorido); (2) `Sidebar.jsx` do webmail — substituído o retângulo azul com ícone Mail; (3) `AdminLayout.jsx` da sidebar admin — mesma troca; (4) `Login.jsx` (Console admin) — logo no hero pane e mobile.

- ✅ **Rastreamento de e-mails via DirectAdmin (07/07/2026)**:
   - **Backend**: `DirectAdminClient.get_email_logs(domain, date_from, date_to, address, state, direction, limit)` faz scraping HTML do `CMD_EMAIL_LOGS` (que o DA autentica via mesma token do painel). BeautifulSoup4 + lxml (adicionados ao `requirements.txt`) parseiam a tabela detectando dinamicamente os headers em pt/en. Helper `_parse_size` converte "63.04 KB" → bytes.
   - **Endpoint**: `GET /api/dominios/{id}/email-logs?date_from=&date_to=&address=&state=&direction=&limit=` — escopo admin (superadmin ou empresa_admin do domínio).
   - **Frontend**: nova página `/console/rastreamento` (`AdminEmailTracking.jsx`) com filtros completos: domínio (dropdown), datas datetime-local, endereço, estado (Entregue/Bounce/Adiado/Congelado), direção (In/Out). Tabela replicando colunas do DA (Direção, Estado, De, Para, Assunto, Tamanho, Data) com badges coloridos por estado. Cards de estatística no topo (Total/Entregues/Bounces/Adiados). Rota registrada em `App.js`.

- ✅ **Catch-all + Spam Central por domínio (07/07/2026)**:
   - **Catch-all**: novos métodos `DirectAdminClient.get_catch_all(domain)` e `set_catch_all(domain, mode, address)` que envolvem `CMD_API_EMAIL_CATCH_ALL`. `mode` aceita `address`|`blackhole`|`fail`|`unset`. Endpoints `GET/PUT /api/dominios/{id}/catch-all` com escopo (superadmin OU empresa_admin do domínio). Modal `CatchAllModal` em `AdminDomains.jsx` com 4 opções radio-card (encaminhar/descartar/rejeitar/desativar) e input do endereço só aparece no modo "address". Botão inbox azul na linha do domínio abre o modal.
   - **Spam Central por domínio**: refatorado `/api/spam/admin/overview`. Se existir uma conta `spam@<dominio>` com senha em cache, o overview usa **ela como fonte única** de spam do domínio (1 conexão IMAP por domínio, muito mais leve) — lê o `INBOX` já que TODA mensagem nessa caixa é spam por definição. Se não existir, cai no comportamento legado (itera contas com senha e lê Junk/Spam de cada). Retorno inclui `mode: "central"|"per-user"` para o frontend saber o que exibir.
   - Endpoints admin/accounts/{id} atualizados: novo helper `_resolve_admin_spam_folder` retorna `INBOX` quando a conta é a central; ao marcar "não é spam" na caixa central, move para pasta `Falso-positivo` (criada on demand) ao invés de INBOX que causaria loop.
   - Testes: `/app/backend/tests/test_catchall_and_spam.py` — 5 novos testes cobrindo auth, 404, validação e helpers DA com mock. **16/16 pass** (novos + regressão).

- ✅ **Login redesenhado no estilo Google (07/07/2026)**:
   - `ClientLogin.jsx` reescrito do zero. Layout: card branco centralizado (max-w-900px) sobre fundo cinza claro (`bg-slate-100`), duas colunas dentro do card — esquerda com logo circular multi-colorido (gradiente conic Google-ish com envelope no meio) + título "Faça login" (fonte 2.25rem font-normal) + subtítulo "Ir para {brandName}"; direita com formulário.
   - **Fluxo em 2 passos** (email → senha), como o Google: primeiro pede só e-mail (com botão "Avançar" pill azul), busca branding do domínio e depois mostra o campo de senha com chip da conta escolhida (avatar+e-mail+chevron) que ao clicar volta pro step de e-mail.
   - **Campos com floating-label estilo Material** (`FloatingInput` novo componente com `forwardRef`): borda cinza 1.5px, borda azul-600 no focus, label sobe pro topo com background branco cortando a borda. Cor de erro vermelho-500.
   - **Rodapé estilo Google**: linha inferior com "Português (Brasil)" à esquerda + "Ajuda / Privacidade / Termos" à direita. Marca discreta "© 2026 {brandName}" ainda mais embaixo.
   - Autocomplete correto (`username` + `current-password`), foco automático no input do step atual, hidden username field no step de senha para password managers funcionarem.
   - Preserva integração de white-label (host-branding + domain-branding) — se o domínio tem logo customizado, substitui o G-colorido pelo logo do cliente.
